...
Component({
  ...
  methods: {
    search() {
      wx.navigateTo({
        url: '/pages/tools/library/search/index?from=library',
      })
    },
    ...
    viewDetail(e) {
      ...
      const url = `/pages/tools/library/detail/index?id=${recordId}&img=${img ? encodeURIComponent(img) : ''}`
      wx.navigateTo({
        url
      })
    },
    ...
    viewMore(e) {
      const type = e.mark.type
      wx.navigateTo({
        url: `/pages/tools/library/top/index?type=${type}`,
      })
    },
